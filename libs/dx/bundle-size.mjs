...
function toKibibytes(record) {
  return Object.fromEntries(Object.entries(record).map(([name, bytes]) => [name, Math.round(bytes / 1024)]))
}

function addSizeComments(json) {
  return json.replace(/^(\s+"(?!schemaVersion")[^"]+": )(\d+)(,?)$/gm, (_, prefix, kibibytes, comma) => {
    const value = Number(kibibytes)
    const size = value < 1024 ? `${value} KiB` : `${Number((value / 1024).toFixed(2))} MiB`
    return `${prefix}${kibibytes}${comma} // ${size}`
  })
}
...
if (Object.keys(apps).length) {
  report.apps = sortRecord(toKibibytes(apps))
  comments.push('Apps contain browser bundle outputs.')
}

report.packages = sortRecord(toKibibytes(packages))
...
comments.push('All values are rounded uncompressed KiB.')
...
const output = await prettier.format(`${comment}${addSizeComments(JSON.stringify(report, null, 2))}`, {
  parser: 'jsonc',
})
