...
function toMebibytes(record) {
  const decimalFactor = 10 ** bundleSizeDecimals

  return Object.fromEntries(
    Object.entries(record).map(([name, bytes]) => [
      name,
      Math.trunc((bytes / 1024 ** 2) * decimalFactor) / decimalFactor,
    ]),
  )
}
...
if (Object.keys(apps).length) {
  report.apps = sortRecord(toMebibytes(apps))
  comments.push('Apps contain browser bundle outputs.')
}

report.packages = sortRecord(toMebibytes(packages))
...
comments.push(`All values are uncompressed MiB and truncated. Decimal precision: ${bundleSizeDecimals}.`)
...
const output = await prettier.format(`${comment}${JSON.stringify(report, null, 2)}`, {
  parser: 'jsonc',
})
