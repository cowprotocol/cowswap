...
import { buildWidgetHttpsUrlOrThrow, buildWidgetUrlQuery, sanitizeWidgetBaseUrl } from './urlUtils'
...
describe.skip('buildWidgetHttpsUrlOrThrow', () => {
  describe('env', () => {
    it('local', () => {
      const url = buildWidgetHttpsUrlOrThrow({ chainId, tradeType, baseUrl: 'http://localhost:3000' })
      expect(url).toEqual('http://localhost:3000/#/1/widget/swap/?')
    })
    it('prod', () => {
      const url = buildWidgetHttpsUrlOrThrow({ chainId, tradeType, baseUrl: 'https://swap.cow.fi' })
  ...
  describe('chainId', () => {
    it('mainnet', () => {
      const url = buildWidgetHttpsUrlOrThrow({ chainId: 1, tradeType })
      expect(url).toEqual('https://swap.cow.fi/#/1/widget/swap/?')
    })
    it('gnosis chain', () => {
      const url = buildWidgetHttpsUrlOrThrow({ chainId: 100, tradeType })
  ...
  describe('theme', () => {
    it('dark', () => {
      const url = buildWidgetHttpsUrlOrThrow({ theme: 'dark', chainId, tradeType })
      expect(url).toEqual('https://swap.cow.fi/#/1/widget/swap/?theme=dark')
    })
    it('light', () => {
      const url = buildWidgetHttpsUrlOrThrow({ theme: 'light', chainId, tradeType })
  ...
  describe('trade assets', () => {
    it('without amounts', () => {
      const url = buildWidgetHttpsUrlOrThrow({
        sell: { asset: 'WETH' },
        buy: { asset: 'COW' },
        chainId,
        tradeType,
...
describe('buildWidgetUrlQuery', () => {
  ...
  it('serializes legacy palette boxShadow when provided', () => {
    const query = buildWidgetUrlQuery({
      theme: {
        baseTheme: 'light',
        primary: '#052b65',
  ...
  it('includes locale in the iframe URL', () => {
    expect(buildWidgetHttpsUrlOrThrow({ chainId, tradeType, locale: 'fr' })).toBe(
