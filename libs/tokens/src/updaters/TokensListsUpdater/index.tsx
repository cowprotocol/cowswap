...
import { shouldInvalidateLastUpdateTime, shouldUseCuratedModeForGeoResponse } from './curatedMode'
  ...
  useEffect(() => {
    ...
    fetch('https://api.country.is', { signal: controller.signal })
      .then(async (res) => {
        ...
        return shouldUseCuratedModeForGeoResponse(responseBody)
      })
      .then((useCuratedMode) => {
        if (isStale) return

        setCuratedListOnly(useCuratedMode)
