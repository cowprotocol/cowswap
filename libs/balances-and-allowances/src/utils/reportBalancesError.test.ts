...
describe('reportBalancesError', () => {
  ...
  it('tags rate-limit (429) errors distinctly', () => {
    reportBalancesError({ error: rateLimitError(), chainId: SupportedChainId.MAINNET, tokenAddressesCount: 12 })
    ...
    expect(params).toMatchObject({ chainId: SupportedChainId.MAINNET, tokenAddressesCount: 12, httpStatus: 429 })
  })

  it('tags non-429 errors as generic multicall errors without an httpStatus tag', () => {
    reportBalancesError({ error: new Error('boom'), chainId: SupportedChainId.GNOSIS_CHAIN, tokenAddressesCount: 1 })
  ...
  it('throttles repeated reports for the same chain + status', () => {
    const error = new Error('boom')
    // A dedicated chain keeps this test's throttle window isolated from the others.
    reportBalancesError({ error, chainId: SupportedChainId.ARBITRUM_ONE, tokenAddressesCount: 1 })
    reportBalancesError({ error, chainId: SupportedChainId.ARBITRUM_ONE, tokenAddressesCount: 1 })
