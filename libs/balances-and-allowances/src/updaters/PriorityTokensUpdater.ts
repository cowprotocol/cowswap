...
export function PriorityTokensUpdater(props: PriorityTokensUpdaterProps) {
  const balancesSwrConfig = useSwrConfigWithPauseForNetwork(props.chainId, BALANCES_SWR_CONFIG, BALANCE_VALIDITY_PERIOD)
  const allowancesSwrConfig = useSwrConfigWithPauseForNetwork(
    props.chainId,
    ALLOWANCES_SWR_CONFIG,
    BALANCE_VALIDITY_PERIOD,
  )
