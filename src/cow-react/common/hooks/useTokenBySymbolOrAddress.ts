...
import { deserializeToken, useFavouriteTokens } from 'state/user/hooks'
import { isSupportedChainId } from 'lib/hooks/routing/clientSideSmartOrderRouter'
import { useWeb3React } from '@web3-react/core'
import { useAtomValue } from 'jotai/utils'
import { tokensListBySymbolState, tokensListState } from '@cow/modules/tokensList/state'
...
export function useTokenBySymbolOrAddress(symbolOrAddress?: string | null): Token | NativeCurrency | null {
  ...
  const tokensByAddress = useAtomValue(tokensListState)
  ...
  return useMemo(() => {
    if (!symbolOrAddress || !isSupportedNetwork || !chainId) {
      return null
    }
  ...
  }, [symbolOrAddress, isSupportedNetwork, nativeCurrency, tokensByAddress, tokensBySymbol, favouriteTokens, chainId])
}
