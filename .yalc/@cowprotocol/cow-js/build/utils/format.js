...
exports.formatPrice = exports.safeFilledToken = exports.safeTokenName = exports.abbreviateString = exports.parseAmount = exports.adjustPrecision = exports.formatAmountFull = exports.formatAmount = exports.formatSmart = exports.stringToBn = void 0;
...
function _formatDecimalsForDisplay(numberToConvert, decimalsSymbol) {
    ...
    const integer = numberToConvert.integerValue(bignumber_js_1.default.ROUND_DOWN);
...
function _decomposeLargeNumberToString(baseUnit, baseUnitsPerRepresentationUnits, decimalsSymbol, thousandsSymbol) {
    ...
    const baseUnitAsDecimal = baseUnit
        .mul(const_1.TEN.pow(new bn_js_1.default(const_1.DEFAULT_LARGE_NUMBER_PRECISION)))
        .div(baseUnitsPerRepresentationUnits);
    const { integerPart, decimalPart, isNegative } = _decomposeBn(baseUnitAsDecimal, const_1.DEFAULT_LARGE_NUMBER_PRECISION, const_1.DEFAULT_LARGE_NUMBER_PRECISION);
    ...
    const formattedDecimal = decimalPart
        .toString(10)
        .padStart(const_1.DEFAULT_LARGE_NUMBER_PRECISION, '0') // Pad the decimal part with leading zeros
        .replace(/0+$/, ''); // Remove the right zeros
    return sign + formattedInteger + decimalsSymbol + formattedDecimal;
}
function _formatNumber(num, thousandsSymbol = const_1.THOUSANDS_SYMBOL) {
...
function _formatSmart({ integerPart, decimalPart, decimalsPadded, isNegative }, smallLimit, isLocaleAware, thousandSeparator) {
    const decimalsSymbol = isLocaleAware ? const_1.DECIMALS_SYMBOL : const_1.DEFAULT_DECIMALS_SYMBOL;
    const thousandsSymbol = !thousandSeparator ? '' : isLocaleAware ? const_1.THOUSANDS_SYMBOL : const_1.DEFAULT_THOUSANDS_SYMBOL;
    const sign = _getSign(isNegative);
    // Is < 1
    if (integerPart.isZero()) {
        ...
        const ourDecimalsAsBigNumber = new bignumber_js_1.default('0.' + decimalsPadded);
        const smallLimitAsBigNumber = new bignumber_js_1.default(smallLimit);
    ...
    if (integerPart.gte(const_1.BN_1T)) {
        return _decomposeLargeNumberToString(integerPart, const_1.BN_1T, decimalsSymbol, thousandsSymbol) + 'T';
    }
    // Anything not above 1T but > 1,000,000,000 denoted as <XXX.xxx>B
    if (integerPart.gte(const_1.BN_1B)) {
        return _decomposeLargeNumberToString(integerPart, const_1.BN_1B, decimalsSymbol, thousandsSymbol) + 'B';
    }
    ...
    if (integerPart.lt(const_1.BN_100K)) {
        // 99,999.3424
        finalPrecision = 4;
    }
    else if (integerPart.lt(const_1.BN_1M)) {
        // 999,999.342
        finalPrecision = 3;
    }
    else if (integerPart.lt(const_1.BN_10M)) {
        // 9,999,999.34
        finalPrecision = 2;
    }
    else {
...
function _decomposeBn(amount, amountPrecision, decimals) {
    ...
    const amountRaw = amount.divRound(const_1.TEN.pow(new bn_js_1.default(amountPrecision - decimals)));
    const integerPart = amountRaw.div(const_1.TEN.pow(new bn_js_1.default(decimals))).abs(); // 165000 / 10000 = 16
    const decimalPart = amountRaw.mod(const_1.TEN.pow(new bn_js_1.default(decimals))).abs(); // 165000 % 10000 = 5000
 ...
 */
function stringToBn(amountStr, additionalPrecision = 0) {
    if (!amountStr || isNaN(+amountStr)) {
        return { amount: null, precision: 0 };
    }
    const bigNumberAmount = new bignumber_js_1.default(amountStr);
    const decimalPlaces = bigNumberAmount.decimalPlaces();
    const amount = new bn_js_1.default(bigNumberAmount.multipliedBy(const_1.TEN_BIG_NUMBER.pow(decimalPlaces)).integerValue().toString(10));
    const precision = decimalPlaces + additionalPrecision;
    return { amount, precision };
}
exports.stringToBn = stringToBn;
function formatSmart(params, _amountPrecision) {
    var _a, _b, _c, _d;
    /*
    ...
    */
    let amount;
    let precision;
    let decimals = const_1.DEFAULT_DECIMALS;
    let smallLimit = const_1.DEFAULT_SMALL_LIMIT;
    ...
    if (bn_js_1.default.isBN(params)) {
        amount = params;
...
function formatAmount(params, _amountPrecision) {
    var _a, _b, _c;
    let amount;
    let precision;
    let decimals = const_1.DEFAULT_DECIMALS;
    ...
    let decimalSymbol;
    if (isLocaleAware) {
        thousandSymbol = const_1.THOUSANDS_SYMBOL;
        decimalSymbol = const_1.DECIMALS_SYMBOL;
    }
    else {
        thousandSymbol = const_1.DEFAULT_THOUSANDS_SYMBOL;
        decimalSymbol = const_1.DEFAULT_DECIMALS_SYMBOL;
    }
    const actualDecimals = Math.min(precision, decimals);
...
function formatAmountFull(params) {
    var _a, _b, _c;
    let amount;
    let precision = const_1.DEFAULT_PRECISION;
    let thousandSeparator = true;
    let isLocaleAware = true;
    if (!params || ('amount' in params && !params.amount))
        return null;
...
exports.formatAmountFull = formatAmountFull;
/**
 ...
 */
function adjustPrecision(value, precision, decimalsSymbol = const_1.DEFAULT_DECIMALS_SYMBOL) {
    if (!value) {
        return '';
    }
...
exports.adjustPrecision = adjustPrecision;
function parseAmount(amountFmt, amountPrecision) {
    if (!amountFmt) {
        return null;
    }
    ...
    if (groups) {
        ...
        const decimalBN = new bn_js_1.default(decimalPart.padEnd(amountPrecision, '0'));
        const factor = const_1.TEN.pow(new bn_js_1.default(amountPrecision));
        return new bn_js_1.default(integerPart || 0).mul(factor).add(decimalBN);
    }
    else {
        return null;
    }
...
function abbreviateString(inputString, prefixLength, suffixLength = 0) {
    ...
    if (inputString.length < _prefixLength + const_1.ELLIPSIS.length + suffixLength) {
        return inputString;
    }
    const prefix = inputString.slice(0, _prefixLength);
    const suffix = suffixLength > 0 ? inputString.slice(-suffixLength) : '';
    return prefix + const_1.ELLIPSIS + suffix;
}
...
exports.safeTokenName = safeTokenName;
function safeFilledToken(token) {
...
exports.safeFilledToken = safeFilledToken;
/**
...
function formatPrice(params) {
    ...
    if (params instanceof bignumber_js_1.default) {
        price = params;
    }
    else {
        price = params.price;
    ...
    if (decimals <= 0 || (!zeroPadding && decimalPart.isZero())) {
        ...
        return integerPartFmt + const_1.DECIMALS_SYMBOL + decimalPartFmt;
    }
}
