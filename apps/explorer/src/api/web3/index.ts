...
export function createWeb3Api(provider?: string): Web3 {
  const _provider = provider || getProviderByNetwork(EvmChains.MAINNET)
...
export function getProviderByNetwork(networkId: EvmChains): string {
...
//
// Callers pass a `SupportedChainId | null` (which now spans Solana) — we keep that wide
// signature to avoid forcing every call site to narrow, and gate non-EVM ids here.
