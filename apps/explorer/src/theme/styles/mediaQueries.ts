...
export const mediaWidthTemplates = Object.keys(mediaQueriesObject).reduce<MediaWidth>((accumulator, size: string) => {
  const [min, max, orientation] = mediaQueriesObject[size as MediaWidthKeys]
  accumulator[size as MediaWidthKeys] = ((a: CSSObject, b: CSSObject, c: CSSObject): FlattenSimpleInterpolation =>
    css`
