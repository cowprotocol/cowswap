...
export function isWrappingTrade(
  ...
  return Boolean(
    (sellCurrency && getIsNativeToken(sellCurrency) && buyCurrency?.wrapped.equals(wethByChain)) ||
      (buyCurrency && getIsNativeToken(buyCurrency) && sellCurrency?.wrapped.equals(wethByChain))
  )
}
