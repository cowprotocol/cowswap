...
function SaucerSvg({ darkMode }: { darkMode: boolean }): ReactNode {
  ...
  return (
    <StyledSaucerSvg viewBox="0 0 528 698" role="presentation">
      <path d={beamPath} fill={colors.beam} fillOpacity={colors.beamOpacity} />
      <path d={sideLeftPath} fill={colors.side} />
      <path d={sideRightPath} fill={colors.side} />
      <path d={domePath} fill={colors.dome} />
      <path d={capPath} fill={colors.cap} />
      {starPaths.map((path, index) => (
        <path key={path} className={`star star--${index + 1}`} d={path} fill={colors.stars} />
      ))}
    </StyledSaucerSvg>
  )
}
...
export function CowSaucerScene({ darkMode }: CowSaucerSceneProps): ReactNode {
  const cowAsset = useMemo(() => (darkMode ? cowDarkIMG : cowLightIMG), [darkMode])
