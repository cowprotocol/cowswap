...
jest.mock('../services/wrapNativeSolana/getSolanaWrapPreview', () => ({
  getSolanaWrapPreview: jest.fn(),
}))
...
const mockGetSolanaWrapPreview = getSolanaWrapPreview as jest.MockedFunction<typeof getSolanaWrapPreview>
...
describe('useSolanaWrapReceiveAmount', () => {
  ...
  it('resolves the preview amount asynchronously for the wrap direction, 1:1 when the account already exists', async () => {
    mockDerivedTradeState.mockReturnValue({
      inputCurrencyAmount: CurrencyAmount.fromRawAmount(SOL, 500n),
    } as unknown as ReturnType<typeof useDerivedTradeState>)
    mockGetSolanaWrapPreview.mockResolvedValue({
      sendAmount: CurrencyAmount.fromRawAmount(SOL, 500n),
      receiveAmount: CurrencyAmount.fromRawAmount(WSOL, 500n),
      transferLamports: 500n,
    })
    ...
    await waitFor(() => expect(result.current).toEqual(CurrencyAmount.fromRawAmount(WSOL, 500n)))

    expect(mockGetSolanaWrapPreview).toHaveBeenCalledWith(CONNECTION, expect.anything(), 500n)
    expect(mockGetSolanaUnwrapPreview).not.toHaveBeenCalled()
  })

  it('reflects the rent-exempt deposit deducted from the received WSOL when the account does not exist yet', async () => {
    mockDerivedTradeState.mockReturnValue({
      inputCurrencyAmount: CurrencyAmount.fromRawAmount(SOL, 10_000n),
    } as unknown as ReturnType<typeof useDerivedTradeState>)
    mockGetSolanaWrapPreview.mockResolvedValue({
      sendAmount: CurrencyAmount.fromRawAmount(SOL, 10_000n),
      receiveAmount: CurrencyAmount.fromRawAmount(WSOL, 1_000n),
      transferLamports: 1_000n,
    })

    const { result } = renderHook(() => useSolanaWrapReceiveAmount())

    await waitFor(() => expect(result.current).toEqual(CurrencyAmount.fromRawAmount(WSOL, 1_000n)))
  })
