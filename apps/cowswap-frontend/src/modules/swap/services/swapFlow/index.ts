...
export async function swapFlow(
  ...
  try {
    ...
    if (isSupportedPermitInfo(permitInfo)) swapConfirmManager.requestPermitSignature()
    ...
    cowEventEmitter.emit(CowEvents.ON_POSTED_ORDER, { orderUid, chainId })
    ...
    swapConfirmManager.transactionSent(orderUid)
