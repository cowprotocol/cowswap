...
export interface EnsureEoaTwapSpenderAllowanceParams {
   ...
   * permit, a permit is preferred. Omit for on-chain-only spenders (e.g. Vault Relayer — setup
   * sell size is only known after the quote).
   */
  permitInfo?: IsTokenPermittableResult
  generatePermitHook?: GeneratePermitHook
  /** Override for on-chain approve stepper step (also used for FundingOrder top-ups). */
  step?: EoaTwapSigningSteps
  /** Override for permit stepper step (defaults to `step`). */
  permitStep?: EoaTwapSigningSteps
  /** Override for USDT-style zero-approve step (defaults to ZeroApproveVaultRelayer, or `step` when set). */
...
export interface EnsureEoaTwapSpenderAllowanceResult {
  usedPermit: boolean
 ...
 * Used for:
 *
 * - Vault Relayer (no permit args): setup fee estimate + buffer pre-placement, and post-quote
 *   funding top-up. Always on-chain — setup sell size is only known after the quote.
 *   See `getEoaTwapPrePlacementAmountToCover`. When approval is needed, the on-chain tx still
 *   approves `maxUint256`. If the Approval event amount from the mined receipt is still too low,
 *   throws "Approved amount is not sufficient!".
 *
 * - ComposableCowPoller (`permitInfo` + `generatePermitHook`): prefer EIP-2612 / Dai-like permit
 *   for the full TWAP sell when supported; otherwise on-chain approve.
 *
 * When permit succeeds, returns `permitData` for the caller to attach as a sell=buy pre-hook.
 */
export async function ensureEoaTwapSpenderAllowance({
  ...
  onSigningStep,
  approvalNeeds,
}: EnsureEoaTwapSpenderAllowanceParams): Promise<EnsureEoaTwapSpenderAllowanceResult> {
  const { needsApproval, needsZeroApproval } = approvalNeeds
  const approveStep = step ?? EoaTwapSigningSteps.ApproveVaultRelayer
  const permitUiStep = permitStep ?? approveStep
  const zeroApproveStep = zeroStep ?? step ?? EoaTwapSigningSteps.ZeroApproveVaultRelayer
...
async function tryGeneratePermitAllowance({
  ...
  onChainFallbackPlan,
  onSigningStep,
}: TryGeneratePermitAllowanceParams): Promise<EnsureEoaTwapSpenderAllowanceResult | null> {
