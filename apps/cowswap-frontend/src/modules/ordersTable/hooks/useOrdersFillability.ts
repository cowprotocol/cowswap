...
export function useOrdersFillability(orders: GenericOrder[]): Record<string, OrderFillability | undefined> {
  ...
  return useMemo(() => {
    return orders.reduce<Record<string, OrderFillability>>((acc, order) => {
      ...
      acc[order.id] = {
        hasEnoughBalance:
          balance !== undefined && order.sellAmount !== undefined ? balance >= BigInt(order.sellAmount) : undefined,
        hasEnoughAllowance:
          allowance !== undefined && order.sellAmount !== undefined ? allowance >= BigInt(order.sellAmount) : undefined,
