...
test.describe('Market Orders', () => {
  ...
  test('Insufficient allowance: asks for approval', async ({ setupTestConditions, swapPage }) => {
    await setupTestConditions({
      chainId: CHAIN_ID,
      tradeType: 'swap',
      sellToken: 'WETH',
  ...
  test('Sell order: balances update in the UI after the order is posted', async ({
    setupTestConditions,
    swapPage,
    tradePage,
    wallet,
